...
def get_train_set(interaction_matrix):
    ...
    for k in range(cv):
        ...
        predict_matrix1=cf_model(train_set)
        test_index1 = np.where(train_set == 0)  ## Each training matrix element is equal to the element position corresponding to 0
        real_score1=interaction_matrix[test_index1]


        train_set_t=train_set.T
        predict_matrix2 = cf_model(train_set_t)
        test_index2 = np.where(train_set_t == 0)  # # Each training matrix element is equal to the element position corresponding to 0
        real_score2 = interaction_matrix.T[test_index2]

        predict_score = np.hstack((predict_matrix1[test_index1], predict_matrix2[test_index2]))
        real_score = np.hstack((real_score1,real_score2))
